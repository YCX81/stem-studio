...
- 实时分离
  ...
  - 可监听任意已生成音轨
  - 12 秒分析窗、3 秒步进、100 ms 同时间轴交叉淡化；持久 WASAPI 队列在窗口间不重开设备
  - PCM 内容寻址窗口缓存，以及按 AirPlay 曲目时间轴构建的连续多轨歌曲缓存；重复曲目校验原声后可绕过 GPU
  - AirPlay 曲目信息与播放进度驱动的逐行同步歌词；LRCLIB 查询在独立线程执行，结果持久缓存，切歌时按曲目 revision 隔离
  - GPU 模型运行在独立子进程中；启动时先完成一整窗 CUDA 推理预热，播放期设有 2.8 秒防卡死硬截止，超时会杀掉子进程并用同帧原声保底
...
- AirPlay/UxPlay 提供歌名、歌手、专辑和播放进度，但不直接提供逐行时间歌词。Stem Studio 使用这些信息向 [LRCLIB](https://www.lrclib.net/docs) 精确查询 LRC，再以 AirPlay 播放位置选择当前句；音频数据不会发送到歌词服务。
- 歌词查询运行在独立后台线程，单次请求最长 10 秒网络超时不会占用音频捕获、GPU 推理或播放线程。查询按“精确缓存、严格候选搜索、在线精确回源”的顺序执行；网络错误按 60 秒限频重试，界面仍保持每 0.5 秒刷新。
- 查询成功的歌词保存在 `data/lyrics`。以后播放相同曲目时直接读取本地缓存，不再联网查询；切歌时 revision、歌名或歌手任一不匹配都会立即隐藏旧歌词。
- LRCLIB 没有匹配结果、歌曲被标记为纯音乐或暂时离线时，界面会显示明确状态，不会用未校验的模糊歌词替代。
...
- 实时是否可持续取决于“单窗 GPU 处理时间是否小于 `3 秒`步进期限”。界面实时显示输入波形、待处理窗口、可播放缓存、欠载次数、单窗耗时、缓存命中和原声保底次数，还会显示隔离推理 PID、模型完整预热状态、2.8 秒硬截止、有效 shifts、自动调优结果、预热/超时保底计数和最慢窗口；正在播放时若积压超过一窗，会优先用原声保底追平实时队列，避免持续过载耗尽预缓冲。
...
```json
{"sequence": 1, "action": "start_airplay", "profile_name": "六轨 · 加吉他/钢琴", "track_count": 6, "hop_seconds": 3, "device_endpoint": "192.168.31.88:4010"}
```
...
| 实时输出模式 | 12 秒窗口处理 | 相对 3 秒步进余量 |
|---|---:|---:|
| 二轨：人声 / 伴奏（六源聚合） | 1.661 秒 | 1.339 秒 |
| 四轨：人声 / 鼓 / 贝斯 / 其他（六源聚合） | 1.991 秒 | 1.009 秒 |
| 六轨完整分轨 | 1.411 秒 | 1.589 秒 |
...
三分钟、六轨、44.1 kHz PCM16 的持久歌曲缓存 I/O 基准占用 `222,266,059` 字节：进程重启后的首次清单校验、PCM 对齐与六轨窗口发布合计 `0.146 秒`，随后 10 次热缓存发布中位数 `0.012 秒`、最慢 `0.014 秒`，均远低于 3 秒实时期限。可用 `python tools/benchmark_song_cache.py --duration-seconds 180 --repeats 10` 复测；基准使用真实缓存几何和文件体积，临时数据结束后自动清理。
...
- `data/lyrics`：按规范化曲目信息和时长索引的同步歌词缓存
...
- 模型切换时显存不足：停止实时会话后重启容器；检查 `data/hardware-profile.json` 的实际显存与自动档位，并确认没有手工设置过高并发。
