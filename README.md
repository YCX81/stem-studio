...
- 歌名、艺术家、专辑和时长只用于缩小候选范围，不能单独触发命中。同名不同版本可以并存；当前 PCM 对不上就安全回退到 GPU。手机音量或服务端母带变化若改变了解码 PCM，也会视为未命中。
...
真机产品验收可运行 `python tools/monitor_live_acceptance.py --timeout-seconds 1800`。监视器要求手机完整首播产生 GPU 窗口和整曲缓存、同曲重播命中歌曲缓存、播放期间欠载保持为零，并在真实音频播放时至少收到一次低于 100 ms 的滑杆更新；结果会持续原子写入 `data/live/acceptance-report.json`。空闲后缓冲自然耗尽不会被误判为播放欠载。
