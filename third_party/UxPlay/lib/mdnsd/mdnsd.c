...
uint32_t mdnsd_select_ipv4(void)
{
    int fd;
    uint32_t addr = 0;
    ...
    configured_addr = getenv("STEM_STUDIO_MDNS_IPV4");
    if (configured_addr && configured_addr[0] != '\0' &&
        inet_pton(AF_INET, configured_addr, &addr) == 1) {
        return addr;
    }
...
int mdnsd_start(mdnsd_t *mdnsd)
{
    ...
    mdnsd->ipv4_addr = mdnsd_select_ipv4();
