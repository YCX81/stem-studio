...
static audio_pcm_callback_t pcm_callback = NULL;
static void *pcm_callback_cls = NULL;
...
static GstFlowReturn pcm_new_sample(GstAppSink *appsink, gpointer user_data) {
    audio_renderer_t *audio_renderer = (audio_renderer_t *) user_data;
    ...
    if (buffer && gst_buffer_map(buffer, &map, GST_MAP_READ)) {
        if (pcm_callback) {
            pcm_callback(pcm_callback_cls, map.data, map.size, audio_renderer->ct);
        }
...
void audio_renderer_set_pcm_callback(audio_pcm_callback_t callback, void *cls) {
    pcm_callback = callback;
    pcm_callback_cls = cls;
}
...
void audio_renderer_init(logger_t *render_logger, const char* audiosink, const bool* audio_sync, const bool* video_sync, const char *artp_pipeline) {
    ...
    for (int i = 0; i < NFORMATS ; i++) {
        ...
        if (pcm_callback) {
            g_string_append(
                ...
                "appsink name=pcm_sink sync=false emit-signals=true max-buffers=32 drop=false");
        } else if (!audio_rtp) {
	...
	if (error) {
        ...
        if (pcm_callback) {
            renderer_type[i]->appsink = gst_bin_get_by_name(GST_BIN(renderer_type[i]->pipeline), "pcm_sink");
            g_signal_connect(renderer_type[i]->appsink, "new-sample", G_CALLBACK(pcm_new_sample), renderer_type[i]);
        }
        switch (i) {
        case 0:
...
void audio_renderer_destroy() {
    ...
    for (int i = 0; i < NFORMATS ; i++ ) {
        ...
        if (renderer_type[i]->appsink) {
            gst_object_unref(renderer_type[i]->appsink);
