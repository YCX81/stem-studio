...
static std::string stem_json_escape(const char *value) {
    std::string escaped;
...
static void write_stem_status(
    const char *state,
    const char *codec,
    const stemstudio::PcmPublisherStats &stats,
    const stemstudio::AudioLevels *levels = NULL,
    ...
    output << "{\"state\":\"" << state
           << "\",\"enabled\":true,\"receiver\":\"Stem Studio\""
           << ",\"codec\":\"" << codec
           << "\",\"pcm_frames\":" << stats.pcm_frames
           << ",\"published_windows\":" << stats.published_windows
           << ",\"sample_rate\":" << stats.geometry.sample_rate
           << ",\"channels\":" << stats.geometry.channels
           << ",\"bits_per_sample\":" << stats.geometry.bits_per_sample
           << ",\"analysis_window_seconds\":" << stats.geometry.window_seconds
           << ",\"hop_seconds\":" << stats.geometry.hop_seconds
           << ",\"track\":{\"revision\":" << track.revision
           ...
           << ",\"position_seconds\":" << track.position_seconds_at(stats.pcm_frames)
           << ",\"duration_seconds\":" << track.duration_seconds()
           << ",\"title\":\"" << stem_json_escape(track.title.c_str())
           << "\",\"artist\":\"" << stem_json_escape(track.artist.c_str())
           << "\",\"album\":\"" << stem_json_escape(track.album.c_str()) << "\"}"
           << ",\"peak_left\":" << (levels ? levels->peak_left : 0.0F)
           << ",\"peak_right\":" << (levels ? levels->peak_right : 0.0F)
           << ",\"rms_left\":" << (levels ? levels->rms_left : 0.0F)
           << ",\"rms_right\":" << (levels ? levels->rms_right : 0.0F)
           << ",\"waveform\":[";
    if (levels) {
        for (std::size_t index = 0; index < levels->waveform.size(); ++index) {
            if (index) output << ',';
            output << levels->waveform[index];
        }
    }
    output << ']';
    if (error) {
        output << ",\"error\":\"" << stem_json_escape(error) << "\"";
    }
    output << "}";
    output.close();
...
static void stem_pcm_process(
    void *,
    const unsigned char *data,
    const size_t size,
    const unsigned char compression_type) {
    try {
        stem_pcm_publisher->append(std::span<const std::byte>(
        ...
        if (stats.published_windows != stem_last_published_windows ||
        ...
        try {
            write_stem_status(
                "error", "unknown", stem_pcm_publisher->stats(), NULL, exception.what());
        } catch (...) {
            /* Never allow a filesystem status error to cross the C callback boundary. */
        }
    }
}

static std::optional<std::string> stem_capture_annotation(
    const stemstudio::PcmWindowDescriptor &descriptor) {
    const auto current_track = stem_track_state.snapshot();
    const auto anchors = stem_track_state.anchors_for_window(
        descriptor.stream_start_frame, descriptor.stream_end_frame);
    return stemstudio::serialize_airplay_capture_annotation(
        descriptor.sequence,
    ...
    if (dmap_tag[0] == 'a' && dmap_tag[1] == 's') {
        ...
        case 'a':
            ...
            case 'l':
                ...
                track_album.assign(metadata, metadata + datalen);
                (void)stem_track_state.set_metadata("asal", track_album, stream_frame);
                break;
            case 'r':
                metadata_text->append("Artist: ");  /*asar*/
                artist.assign(metadata, metadata + datalen);
                (void)stem_track_state.set_metadata("asar", artist, stream_frame);
                if (render_coverart) {
        ...
        track_title.assign(metadata, metadata + datalen);
        (void)stem_track_state.set_metadata("minm", track_title, stream_frame);
    }
...
extern "C" void audio_set_progress(void *cls, uint32_t *start, uint32_t *curr, uint32_t *end) {
    const auto stream_frame = stem_pcm_publisher
        ? stem_pcm_publisher->stats().pcm_frames
        : std::uint64_t{};
    (void)stem_track_state.update_progress(*start, *curr, *end, stream_frame);
    rtptime_start = *start;
    rtptime = *curr;
    rtptime_end = *end;
    display_progress(rtptime_start, rtptime, rtptime_end);
...
extern "C" void audio_set_metadata(void *cls, const void *buffer, int buflen) {
    ...
    int datalen;
    int count = 0;
    const auto metadata_stream_frame = stem_pcm_publisher
        ? stem_pcm_publisher->stats().pcm_frames
        : std::uint64_t{};
